...
let searchTimeout, currentType = "movie",
currentPage = 1,
...
isLoading || (isLoading = !0, showLoading(), fetch(`${BASE_URL}/discover/${currentType}?api_key=${API_KEY}&page=${e}&sort_by=${currentSort}&with_genres=${currentGenre}&year=${currentYear}&with_original_language=${currentLanguage}`).then(e => e.json()).then(t => {
...
let a = document.getElementById("content-view");
a && (t && (a.innerHTML = ""), e.forEach(e => {
    let t = document.createElement("div");
    t.className = "movie-card";
    let n = new Date(e.release_date || e.first_air_date).toLocaleDateString("en-US", {
            year: "numeric",
            month: "long",
            day: "numeric"
        }),
    ...
    `, t.addEventListener("click", () => navigateToDetail(e.id)), a.appendChild(t)
}))
}
...
let t = `${BASE_URL}/search/${currentType}?api_key=${API_KEY}&query=${encodeURIComponent(e)}`;
fetch(t).then(e => e.json()).then(e => displaySearchResults(e.results)).catch(e => console.error("Error:", e))
}

function displaySearchResults(e) {
let t = document.querySelector(".search-results"),
    a = document.querySelector("#searchInput");
t.innerHTML = "", a.value.trim() && e.length > 0 ? (e.slice(0, 5).forEach(e => {
    let a = document.createElement("div");
    a.className = "search-result-item";
    let n = new Date(e.release_date || e.first_air_date).getFullYear();
    a.innerHTML = `
...
fetch(`${BASE_URL}/genre/${currentType}/list?api_key=${API_KEY}`).then(e => e.json()).then(e => populateGenreFilter(e.genres)).catch(e => console.error("Error:", e))
}

function populateGenreFilter(e) {
let t = document.getElementById("genreFilter");
t && (t.innerHTML = '<option value="">Genre</option>', e.forEach(e => {
    let a = document.createElement("option");
    a.value = e.id, a.textContent = e.name, t.appendChild(a)
}))
}
...
e && [{
    ...
    let a = document.createElement("option");
    a.value = t.value, a.textContent = t.label, e.appendChild(a)
})
}

function fetchLanguages() {
fetch(`${BASE_URL}/configuration/languages?api_key=${API_KEY}`).then(e => e.json()).then(e => populateLanguageFilter(e)).catch(e => console.error("Error:", e))
}

function populateLanguageFilter(e) {
let t = document.getElementById("languageFilter");
t && (e.sort((e, t) => e.english_name.localeCompare(t.english_name)), t.innerHTML = '<option value="">Language</option>', e.forEach(e => {
    let a = document.createElement("option");
    a.value = e.iso_639_1, a.textContent = e.english_name, t.appendChild(a)
}))
}
...
window.innerHeight + window.scrollY >= document.body.offsetHeight - 500 && !isLoading && fetchData(++currentPage)
}
...
"movie" === currentType ? (movieToggle.classList.add("active"), tvToggle.classList.remove("active"), indicator.style.transform = "translateX(0)") : (movieToggle.classList.remove("active"), tvToggle.classList.add("active"), indicator.style.transform = "translateX(100%)")
}
...
let e = document.getElementById("messageBar");
...
e.preventDefault(), currentType = "movie", setActiveToggle()
}), tvToggle.addEventListener("click", e => {
e.preventDefault(), currentType = "tv", setActiveToggle()
}), setActiveToggle(), searchInput.addEventListener("input", () => {
...
let e = document.getElementById("searchInput"),
    ...
    a = document.getElementById("sortFilter"),
    n = document.getElementById("genreFilter"),
    r = document.getElementById("yearFilter"),
    i = document.getElementById("languageFilter"),
    l = document.getElementById("movieToggle"),
    o = document.getElementById("tvToggle");
e && t && (e.addEventListener("input", e => {
    clearTimeout(searchTimeout), searchTimeout = setTimeout(() => search(e.target.value), 300)
}), t.addEventListener("click", () => search(e.value)), document.addEventListener("click", e => {
...
}), l && o && (l.addEventListener("click", e => {
...
t.src = "legacy-script.js", document.head.appendChild(t)
}
...
t.src = e, t.alt = "Description of the image", t.className = "dynamic-image", document.getElementById("image-container").appendChild(t)
}
...
let e = document.querySelectorAll("img[data-src]"),
    t = new IntersectionObserver((e, t) => {
        e.forEach(e => {
            if (e.isIntersecting) {
                let a = e.target;
                a.src = a.dataset.src, a.classList.remove("lazy"), t.unobserve(a)
            }
        })
    });
...
e.forEach(e => {
    let t = document.createElement("div");
    t.className = "movie-card";
    let a = new Date(e.release_date || e.first_air_date).toLocaleDateString("en-US", {
            year: "numeric",
            month: "long",
            day: "numeric"
        }),
    ...
    t.innerHTML = `
        ...
        <div class="movie-rating">${r}%</div>
    `, t.addEventListener("click", () => navigateToDetail(e.id)), n.appendChild(t)
}), t && (a.innerHTML = ""), a.appendChild(n), lazyLoadImages()
}
async function fetchDataWithExpiry(e, t = 60) {
let a = `tmdb_${e}`,
    n = JSON.parse(localStorage.getItem(a));
if (n && Date.now() - n.timestamp < 6e4 * t) return console.log("Loading data from cache with expiry check"), n.data;
try {
    let r = await fetch(`https://api.themoviedb.org/3/${e}`),
        i = await r.json();
    return localStorage.setItem(a, JSON.stringify({
        data: i,
        timestamp: Date.now()
    })), console.log("Data cached in localStorage with expiry"), i
} catch (l) {
